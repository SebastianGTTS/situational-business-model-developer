...
export class MergeService {
  ...
  async getSelectedExpertModels(companyModelId: string) {
    const companyModel = await this.companyModelService.get(companyModelId);
    return this.expertModelService.getList({_id: {$in: Object.keys(companyModel.expertModelTraces)}});
  }

  async getUnselectedExpertModels(companyModelId: string) {
    const companyModel = await this.companyModelService.get(companyModelId);
    const getRootFeatureIds = (canvasDefinition: CanvasDefinition) => {
      const rootFeatureIds: string[] = [];
      canvasDefinition.rows.forEach((row) => row.filter((cell) => !cell.isSpacer).forEach((cell) => {
    ...
    result.docs = result.docs.filter((expertModel: ExpertModel) => {
      const ids = getRootFeatureIds(expertModel.$definition);
      return ids.length === definitionIds.length && ids.every((id) => definitionIds.includes(id));
    });
    return result;
  }
  ...
  async addFeatureMerge(
    ...
    companyModel.addExpertModelTrace(expertModel, expertModelFeatureId, newFeature.id);
    return this.companyModelService.save(companyModel);
  }

  async addAllSubfeaturesMerge(featureModelId: string, expertModelId: string, expertModelFeatureId: string) {
    const expertModel = await this.expertModelService.get(expertModelId);
    const companyModel = await this.companyModelService.get(featureModelId);
    ...
    subfeatures.filter(
      (feature) => !companyModel.expertModelTraces[expertModelId].expertFeatureIdMap[feature.id]
    ).forEach((feature) => {
      const newFeature = companyModel.addFeature(
        {
          name: feature.name, description: feature.description, type: feature.type, subfeatureConnections: feature.subfeatureConnections
        }, companyModel.expertModelTraces[expertModelId].expertFeatureIdMap[feature.parent.id]
      );
      companyModel.addExpertModelTrace(expertModel, feature.id, newFeature.id);
    });
    return this.companyModelService.save(companyModel);
  }

  async addTrace(featureModelId: string, featureId: string, expertModelId: string, expertModelFeatureId: string) {
    const expertModel = await this.expertModelService.get(expertModelId);
    const companyModel = await this.companyModelService.get(featureModelId);
    companyModel.addExpertModelTrace(expertModel, expertModelFeatureId, featureId);
    return this.companyModelService.save(companyModel);
  }

  async deleteTrace(featureModelId: string, featureId: string, expertModelId: string) {
    const companyModel = await this.companyModelService.get(featureModelId);
  ...
  async addRelationship(featureModelId: string, relationshipType: RelationshipType, fromFeatureId: string, toFeatureId: string) {
    const companyModel = await this.companyModelService.get(featureModelId);
    companyModel.addRelationship(relationshipType, fromFeatureId, toFeatureId);
    return this.companyModelService.save(companyModel);
  }

  async removeRelationship(featureModelId: string, relationshipType: RelationshipType, fromFeatureId: string, toFeatureId: string) {
    const companyModel = await this.companyModelService.get(featureModelId);
    companyModel.removeRelationship(relationshipType, fromFeatureId, toFeatureId);
    return this.companyModelService.save(companyModel);
  }
}
