...
@NgModule({
  providers: [
    ...
    CanvasMetaModelApiService,
    {
      ...
      useFactory: (canvasMetaModelService: CanvasMetaModelService) => () => canvasMetaModelService.init(),
...
export class CanvasMetaModelModule {
  ...
  constructor(@Optional() @SkipSelf() databaseModule?: CanvasMetaModelModule) {
    if (databaseModule) {
      throw new Error('CanvasMetaModelModule is already loaded. Import it in the AppModule only.');
    }
  }
