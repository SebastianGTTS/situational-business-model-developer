...
export class ExpertModel extends FeatureModel implements ExpertModelInit {
  ...
  static readonly defaultIcon: IconInit = { icon: 'bi-columns-gap' };
  ...
  constructor(
    ...
    if (init != null && init.icon == null) {
      init.icon = ExpertModel.defaultIcon;
    }
    ...
    if (entry != null) {
      this.domains =
        ...
        this.domains;
      this.version = entry.version ?? this.version;
    } else if (init != null) {
      this.domains =
        init.domains?.map((domain) => new Domain(undefined, domain)) ??
        this.domains;
      this.version = init.version ?? this.version;
    } else {
