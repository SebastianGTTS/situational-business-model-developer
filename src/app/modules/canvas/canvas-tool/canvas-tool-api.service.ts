...
export class CanvasToolApiService implements ModuleApiService {
  createCanvas(input: MethodExecutionInput): void {
    const stepInfo = CanvasToolApiService.getStepInfo(input);
  ...
  editCanvas(input: MethodExecutionInput): void {
    const stepInfo = CanvasToolApiService.getStepInfo(input);
    const reference: InstanceArtifactData = input.inputStepArtifacts[0].data
      .data as InstanceArtifactData;
  ...
  private static getStepInfo(input: MethodExecutionInput): StepInfo {
    return {
