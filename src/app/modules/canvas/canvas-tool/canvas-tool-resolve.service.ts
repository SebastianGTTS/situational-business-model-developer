...
export class CanvasToolResolveService {
  ...
  resolveCreateCanvas(
    ...
    const artifactData = CanvasToolResolveService.getArtifactData(
      companyModelId,
      instanceId
    );
  ...
  async resolveCreateCanvasManually(
    ...
    await this.runningProcessService.addInternalArtifact(
      runningProcessId,
      artifactId,
      CanvasToolResolveService.getArtifactData(companyModelId, instanceId)
    );
  }
