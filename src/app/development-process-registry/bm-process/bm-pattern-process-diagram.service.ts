...
import { BmPatternProcess, BmProcessDiagram } from './bm-pattern-process';
import { BmPatternProcessDiagramModelerService } from './bm-pattern-process-diagram-modeler.service';
...
export class BmPatternProcessDiagramService {
  constructor(
    private bmProcessDiagramModelerService: BmPatternProcessDiagramModelerService,
  ...
  async hasEmptyTasks(bmProcess: BmPatternProcess): Promise<boolean> {
    ...
    const tasks = this.bmProcessDiagramModelerService.getTasks(modeler);
    const result = tasks.some((task) => !(task.id in bmProcess.decisions));
    this.bmProcessDiagramModelerService.abortModeling(modeler);
    return result;
  }
  ...
  async getEmptyTasks(
    ...
    const tasks = this.bmProcessDiagramModelerService.getTasks(modeler);
    const result = tasks.filter((task) => !(task.id in bmProcess.decisions));
    this.bmProcessDiagramModelerService.abortModeling(modeler);
