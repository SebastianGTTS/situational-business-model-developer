...
export class BmPatternProcessDiagramModelerService extends ModelerService {
  /**
   ...
   */
  async initModeling(bmProcess?: BmPatternProcess): Promise<BpmnModeler> {
    const processModeler =
      BmPatternProcessDiagramModelerService.getBmProcessModeler();
    if (bmProcess != null) {
   ...
   * @param modeler
   */
  getTasks(modeler: BpmnModeler): BpmnFlowNode[] {
    return modeler
      .get('elementRegistry')
      .filter(
        (element) =>
          BpmnUtils.isTask(element) || BpmnUtils.isCallActivity(element)
      ) as BpmnFlowNode[];
  }
