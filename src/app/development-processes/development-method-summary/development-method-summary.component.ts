...
import { Decision, GroupSummary } from '../../development-process-registry/bm-process/decision';
...
import { MetaModelApi, MetaModelDefinition } from '../../development-process-registry/meta-model-definition';
...
export class DevelopmentMethodSummaryComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes.inputArtifacts && changes.inputArtifacts.currentValue != null) {
      const inputArtifacts: StepInputArtifact[] = changes.inputArtifacts.currentValue;
      ...
      for (let index = 0; index < inputArtifacts.length; index++) {
        ...
        if (stepArtifact.data.type === ArtifactDataType.REFERENCE) {
          ...
          this.metaModelDefinitions[index] = this.metaModelService.getMetaModelDefinition(reference.type);
        }
      }
    }
  }
