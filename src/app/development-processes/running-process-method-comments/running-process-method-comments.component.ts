...
export class RunningProcessMethodCommentsComponent {
  ...
  @ViewChild('editCommentModal', {static: true}) editCommentModal: any;
  ...
  _addComment(formGroup: FormGroup) {
    this.addComment.emit(new Comment(formGroup.value));
    formGroup.reset();
  }

  _editComment(comment: Comment) {
    this.modalComment = comment;
    this.modalReference = this.modalService.open(this.editCommentModal, {size: 'lg'});
  }

  _updateComment(formGroup: FormGroup) {
