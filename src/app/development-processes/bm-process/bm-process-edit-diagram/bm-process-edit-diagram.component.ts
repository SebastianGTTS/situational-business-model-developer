...
import { BmPatternProcessDiagramModelerService } from '../../../development-process-registry/bm-process/bm-pattern-process-diagram-modeler.service';
  ...
  implements DiagramComponentInterface, OnChanges
{
  ...
  openDevelopmentMethodSummary(methodElement: BpmnFlowNode): void {
    ...
    developmentMethodSummaryModal.name = this.modalElement.businessObject.name;
    developmentMethodSummaryModal.methodDecision =
      this.bmProcess.decisions[this.modalElement.id];
  }
  ...
  async checkComplete(): Promise<void> {
    ...
    this.incompleteNodes = await this.bmProcessService.getEmptyTasks(
      this.bmProcess
    );
  }
}
