  ...
  implements OnInit, OnChanges, OnDestroy, Updatable
{
  ...
  @Output() submitSituationalFactorsForm = new EventEmitter<UntypedFormArray>();

  situationalFactorsForm: UntypedFormGroup = this.fb.group({
  ...
  submitForm(): void {
    this.submitSituationalFactorsForm.emit(
      this.situationalFactorsForm.get('situationalFactors') as UntypedFormArray
    );
  }
